...
export default function StoreDetailPage({
  ...
  const storeId = unwrappedParams.id as Id<"stores">;
  ...
  if (store === undefined) {
    return (
      <div className={styles.adminContent} style={{ color: "#6b6540" }}>
        Loading…
      </div>
    );
  }
  ...
  return (
    <div className={styles.adminContent}>
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          >
            <Link
              href="/admin/stores"
              style={{ fontSize: 13, color: "#6b6540" }}
            >
              ← Sellers
            </Link>
          </div>
      ...
      {store.rejectionReason && (
        <div
          style={{
            background: "rgba(166, 62, 38, 0.06)",
            border: "1px solid rgba(166, 62, 38, 0.2)",
            borderRadius: 10,
            padding: "14px 18px",
            marginBottom: 24,
          }}
        >
          ...
          <div style={{ fontSize: 13, color: "#6b6540" }}>
            {store.rejectionReason}
          </div>
        </div>
      )}
      ...
      <div className={styles.detailLayout}>
        <div className={styles.detailMain}>
          ...
          {(store.banner || store.logo) && (
            <div className={styles.adminCard}>
              <div className={styles.adminCardHeader}>
                <h3 className={styles.adminCardTitle}>Images</h3>
              </div>
              ...
              {store.physicalAddress && (
                <div className={styles.infoRow}>
                  ...
                  <span className={styles.infoRowValue}>
                    {store.physicalAddress}, {store.city}, {store.stateName}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className={styles.adminCardBody}>
              {store.email && (
                <div className={styles.infoRow}>
                  ...
                  <span className={styles.infoRowValue}>{store.email}</span>
                </div>
              )}
              {store.phone && (
                <div className={styles.infoRow}>
                  <span className={styles.infoRowLabel}>Phone</span>
                  <span className={styles.infoRowValue}>{store.phone}</span>
                </div>
              )}
              {store.bankName && (
                <div className={styles.infoRow}>
                  <span className={styles.infoRowLabel}>Bank</span>
                  <span className={styles.infoRowValue}>{store.bankName}</span>
                </div>
              )}
              {store.accountName && (
                <div className={styles.infoRow}>
                  <span className={styles.infoRowLabel}>Account Name</span>
                  <span className={styles.infoRowValue}>
                    {store.accountName}
                  </span>
                </div>
              ...
              {store.accountNumber && (
                <div className={styles.infoRow}>
                  <span className={styles.infoRowLabel}>Account No.</span>
                  <span
                    className={styles.infoRowValue}
          ...
          <div className={styles.adminCard}>
            <div className={styles.adminCardHeader}>
              ...
              <Link
                href={`/admin/products?store=${storeId}`}
                  ...
                  <tbody>
                    {store.products.slice(0, 5).map((p) => (
                      <tr key={p._id}>
                        ...
                        <td>
                          <span
                            className={`${styles.badge} ${styles[p.status ?? "approved"]}`}
                          >
                            {p.status ?? "active"}
                          </span>
                        </td>
        ...
        <div className={styles.detailSidebar}>
          ...
          <div className={styles.adminCard}>
            <div className={styles.adminCardHeader}>
              <h3 className={styles.adminCardTitle}>Owner</h3>
            </div>
            <div className={styles.adminCardBody}>
              {store.owner ? (
                <>
          ...
          <div className={styles.adminCard}>
            <div className={styles.adminCardHeader}>
              <h3 className={styles.adminCardTitle}>KYC Verification</h3>
            </div>
            <div className={styles.adminCardBody}>
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ marginTop: 8 }}>
                  {store.verificationStatus ? (
                    <span
                      className={`${styles.badge} ${styles[store.verificationStatus]}`}
                    >
              ...
              {store.businessRegistrationFile && (
                <div className={styles.infoRow}>
                  ...
                  <a
                    href={store.businessRegistrationFile}
                    target="_blank"
                    style={{ fontSize: 12, color: "#636d21" }}
                  >
                    View doc ↗
                  </a>
                </div>
